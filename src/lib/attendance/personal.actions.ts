...
import AttendanceRecord, { AttendanceRecordWithId, RawAttendanceRecord } from 'src/models/attendance-record';

export async function createAttendanceRecord(attendanceRecordData: RawAttendanceRecord) {
    const session = await getSession();
    if (!session) {
        return Promise.reject("You need to be logged in to create a attendance record");
    }
    try {
        await dbConnect();
...
export async function getAttendanceRecords() :Promise<AttendanceRecordWithId[]> {
    try {
        const session = await getSession();
        if (!session) {
...
export async function updateAttendanceRecord(recordId:string,present: boolean) :Promise<string> {
    const session = await getSession();
    if (!session) {
        return Promise.reject("You need to be logged in to update a attendance record");
    }
    try {
        await dbConnect();
        // Validate the attendance record data
...
export async function deleteAttendanceRecord(recordId:string) {
    const session = await getSession();
    if (!session) {
        return Promise.reject("You need to be logged in to delete a attendance record");
    }
    try {
        await dbConnect();
        // Validate the attendance record data
...
export async function forceUpdateAttendanceRecord(recordId:string,attendanceRecordData: Partial<RawAttendanceRecord>) {
    const session = await getSession();
    if (!session) {
        return Promise.reject("You need to be logged in to update a attendance record");
    }
