...
export async function voteOnPoll(pollId:string, optionId:string):Promise<PollType> {
    const session = await getSession();
    if (!session) {
...
export async function getPollsVotedByUser(userId:string) :Promise<PollType[]>  {
    try {
        await dbConnect();
        const polls = await Poll.find({ votes: {
            $in: [{
                $elemMatch: {
                    userId: userId
                }
...
export async function getPollsCreatedByUser(userId:string) :Promise<PollType[]>  {
    try {
        await dbConnect();
