...
export default function UserList({ initialUsers, initialHasMore }: UserListProps) {
    ...
    const loadMoreUsers = async () => {
        if (hasMore) {
            setLoading(true);
            await axios.get(`/api/users?query=${searchParams.get('query') || ""}&offset=${offset}`)
    ...
    return (
        <>
            ...
            <div className="flex justify-center w-full mt-4">
                {hasMore ? <Button onClick={loadMoreUsers} disabled={loading} className="w-1/2 mx-auto">
                    {loading && <Loader2 className="animate-spin" />}
                    {!loading && 'Load More'}
                </Button> : <div className="w-1/2 mx-auto">
                    No more users
                </div>}
            </div>
